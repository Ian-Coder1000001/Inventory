    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <main style={{ maxWidth: '1100px', margin: '0 auto', padding: '40px 24px 160px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '16px' }}>
          {products.map((product, i) => (
            <div key={product._id} className="fade-up" style={{ animationDelay: `${i * 0.04}s`, opacity: 0, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '12px', overflow: 'hidden' }}>
              ...
              <div style={{ padding: '16px' }}>
                ...
                {/* Actions Footer - Accessible to Manager or Owner */}
                {(session?.role === 'manager' || session?.role === 'owner') && (
                  <div style={{ display: 'flex', gap: '8px' }}>
                    {/* Mark Sold only for Manager */}
                    {product.status === 'in_stock' && session?.role === 'manager' && (
                      <button
                    ...
                    <button
                      ...
                      style={{ flex: session?.role === 'owner' ? 1 : 'unset', padding: '9px 12px', background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: '7px', color: 'var(--text-dim)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
        ...
        {/* Activity Feed - Ian sees this at the bottom */}
      ...
      {showAddProduct && (
        <AddProductModal
          ...
          onAdded={(p) => { setProducts(prev => [p, ...prev]); setShowAddProduct(false); fetchData(); }}
        />
      )}
      {sellTarget && (
        <SellModal
          ...
          onSold={(updated) => { setProducts(prev => prev.map(p => p._id === updated._id ? updated : p)); setSellTarget(null); fetchData(); }}
        />
      )}
    </div>
  );
