...
export default function Navbar({ email, role }) {
  ...
  return (
    <nav style={{ borderBottom: '1px solid var(--border)', background: 'rgba(8,11,16,0.9)', backdropFilter: 'blur(12px)', position: 'sticky', top: 0, zIndex: 40, width: '100%' }}>
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '0 20px', height: '56px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          {email && (
            <div style={{ display: 'none', alignItems: 'center', gap: '8px', '@media (min-width: 640px)': { display: 'flex' } }}>
               {/* Hidden on very small screens to save space */}
              <span style={{ color: 'var(--text-muted)', fontSize: '12px', fontFamily: 'DM Mono, monospace' }}>{email.split('@')[0]}</span>
            </div>
          )}
          <span style={{
          ...
          <button
            ...
            style={{ background: 'none', border: '1px solid var(--border)', borderRadius: '6px', padding: '5px 10px', color: 'var(--text-muted)', fontSize: '12px', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}
          >
            Sign out
          </button>
        </div>
